...
import Step2SelectComponents from './steps/Step2SelectComponents';
import Step2ApplicationDeployments from './steps/Step2ApplicationDeployments';
import Step3DatabaseConfiguration from './steps/Step3DatabaseConfiguration';
import Step4MonitoringEndpoints from './steps/Step4MonitoringEndpoints';
import Step7TeamAssignment from './steps/Step7TeamAssignment';
import Step8VerificationAndReview from './steps/Step8VerificationAndReview';
...
const stepsMeta = [
  ...
  { id: 2, name: 'Components', endpoint: 'Frontend, Backend, DB', icon: CheckCircle2 },
  { id: 3, name: 'Deployments', endpoint: 'Target URLs', icon: Server },
  { id: 4, name: 'Database', endpoint: 'DB Metadata (Optional)', icon: Cpu },
  { id: 5, name: 'Endpoints', endpoint: '/health, /ready, /metrics', icon: Activity },
  { id: 6, name: 'Team', endpoint: 'POST /api/users/invite', icon: Users },
  { id: 7, name: 'Verification', endpoint: 'POST /api/projects/:id/verify', icon: CheckCircle2 }
];
...
const mockFixtureWizardState = {
  ...
  step2Deployments: {
    frontendUrl: 'https://app.company.com',
...
export default function ProjectOnboardingWizard({ isOpen, onClose, onProjectCreated }) {
  ...
  const handleCompleteSetup = async () => {
    ...
    try {
      ...
      onClose();
      navigate(res.redirectUrl || `/project/${res.projectId}/dashboard`);
    } catch (err) {
      const msg = err?.response?.data?.message || err?.message || 'Failed to complete project onboarding.';
  ...
  const renderStepComponent = () => {
    switch (currentStep) {
      ...
      case 2:
        return <Step2SelectComponents selectedComponents={wizardState.componentsSelected || { frontend: true, backend: true, database: false }} onChange={(c) => updateStepData('componentsSelected', c)} />;
      case 3:
        return <Step2ApplicationDeployments formData={wizardState.step2Deployments} onChange={(d) => updateStepData('step2Deployments', d)} />;
      case 4:
        return <Step3DatabaseConfiguration formData={wizardState.step3Database} onChange={(d) => updateStepData('step3Database', d)} />;
      case 5:
        return <Step4MonitoringEndpoints formData={wizardState.step4Endpoints} onChange={(d) => updateStepData('step4Endpoints', d)} />;
      case 6:
        return <Step7TeamAssignment data={wizardState.step7} updateData={(d) => updateStepData('step7', d)} />;
      case 7:
        return <Step8VerificationAndReview wizardState={wizardState} projectId={createdProjectId} />;
      default:
        return null;
    }
  };
